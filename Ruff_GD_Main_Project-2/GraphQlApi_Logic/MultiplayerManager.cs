...
namespace RuffGdMainProject.DataClasses
{
    public class MultiplayerManager : Node2D
    {
        ...
        public void RecieveSubscriptionData(SubscriptionData data)
        {
            ...
            {
                Logger.UiLogger.Log(Logger.LogLevel.INFO, "It's a turn change event now we have to check who's turn it is");
                ...
                GridManager.GM.TM.CheckIfChangeTurn(player);
            }
            else if(eventName.Equals("MOVE"))
            {
                Logger.UiLogger.Log(Logger.LogLevel.INFO, "Unit moved!!!");
            ...
            else if(eventName.Equals("ATTACK"))
            {
                ...
                if(card.CardName.Equals("Bubbles' Pick 'n' Drop"))
                {
